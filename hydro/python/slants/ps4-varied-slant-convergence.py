...
target_file = "data/ps4-all-configs-v3.pkl"
...
for config in configurations[1:]:
   ...
   config["DPs by nmk"] = []
   for nmk in nmks:
      am, dp = solve_prob(config, 50, 3, nmk)
      config["AMs by nmk"].append(am)
      config["DPs by nmk"].append(dp)
      update_data_file(configurations, target_file)
   print("Finished nmk variation for config " + config["name"])