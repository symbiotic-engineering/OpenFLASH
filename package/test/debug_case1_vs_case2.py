...
for i, sub in enumerate(SUBSETS):
    ...
    ALL_CONFIGS[f"config3_pair{i+1}_case1"] = {
        ...
        "heaving_map": [False, True], "body_map": [0, 1], "m0": 1.0, "NMK": [15]*3, 
...
def analyze_openflash(config_name):
    ...
    pot_dict = engine.calculate_potentials(
        problem, x, m0, spatial_res=50, sharp=False, 
        R_range=p["R_range"], Z_range=p["Z_range"]
    )
    return pot_dict['phi'], pot_dict['R'], pot_dict['Z'], A, A_inv, b, freq

def analyze_capytaine(config_name, R_grid, Z_grid, omega_val):
    p = ALL_CONFIGS[config_name]
    a, d, h, m0 = p["a"], p["d"], p["h"], p["m0"]
    active_heaving_map = [False, True] # ONLY heave Body 0
