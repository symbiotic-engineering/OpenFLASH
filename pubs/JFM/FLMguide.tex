...
So far, we have only considered geometries in the form of a series of surface-piercing compound annular cylinders, as shown in Fig.~\ref{fig:Diagram}. In this section, we will discuss how MEEM can be used to approximate the hydrodynamic forces on surface-piercing axisymmetric bodies. This generalization is important for modeling body shapes that are seen in engineering applications. The CorPower WEC, WaveBot, AquaHarmonics WEC, LUPA WEC float, and RM3 WEC float are all examples of geometries that can be modeled with this extension. In literature, toroidal and spherical bodies have also been modeled using MEEM. However, little discussion has been given on the accuracy of this approximation, how slanted geometries should be discretized, and how the steepness of the slanted geometry influences the convergence of the MEEM solution. In this section, we will address these topics. \textcolor{blue}{add citations}
...
To determine the number of subdivisions needed to accurately model a slant with a given steepness, we compared the hydrodynamic coefficients from Capytaine and MEEM for a variety of steepnesses and number of subdivisions. As shown in Fig.~\ref{fig:slope-dependence}, steeper slants (smaller angle $\beta$) require more subdivisions for the same level of accuracy, and the error in the approximation is roughly inversely proportional to the number of subdivisions. Convergence of added mass with respect to number of subdivisions appears slower than that of damping. It is achievable for damping to approach a similar level of error/difference to that of purely cylindrical (non-slanted) configurations. Added mass can approach acceptable ($<3\%$) errors, but those are larger than those in the cylindrical case. 
