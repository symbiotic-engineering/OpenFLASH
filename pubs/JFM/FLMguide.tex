...
Early analytical solutions for the wave force on vertical cylindrical bodies under linear potential flow theory include solutions for infinite cylinders in infinite depth (\cite{havelock_forces_1936}) and for bottom-mounted cylinders in finite depth (\cite{fuchs_wave_1954}).
...
With the growing need for efficient hydrodynamic analysis tools to enable advanced design procedures like optimization, interest in MEEM has resurged, particularly in the offshore renewable energy space. \cite{PAVLIDOU2022510} performed multi-objective optimization for a two-body wave energy converter (WEC) using MEEM to compute the hydrodynamic coefficients of a cylinder. \cite{zhang_hydrodynamic_2016} optimized the shape of the wetted surface of a point absorber WEC using MEEM to model cylindrical, hemispherical, paraboloid, and conical geometries. \cite{mavrakos2012hydrodynamic} used MEEM to compute the first-order and mean second-order loads on a vertical axisymmetric oscillating water column WEC. However, the benefits of MEEM over traditional BEM solvers remain unclear, as the tradeoffs between accuracy and computational performance of the methods are seldom compared in the literature. Additionally, previous work does not directly address the convergence behavior and angle-dependent discretization of slanted geometries, making its ability to accurately model more general geometries uncertain.

In this work, we present a unifying framework for MEEM that arranges equations into an organized sparse matrix structure. This framework is used to model an arbitrary number of fixed or heaving surface-piercing annular cylinders with continuous and radially-monotonic body profiles. The method’s convergence behavior, ability to accurately approximate slanted geometries, and computational advantages over the BEM solver Capytaine are explored. The authors build upon their preliminary work (\cite{mccabe_open-source_2024}) and have developed the open-source Python package called \texttt{OpenFLASH} (Flexible Library for Analytical and Semi-analytical Hydrodynamics) (\cite{best_openflash_2026}) that is used for numerical studies in this work.
...
While this geometry may seem niche, it can be used to model more general geometries, such as the CorPower WEC (\cite{de2016techno}), WaveBot WEC (\cite{strofer2023control}), AquaHarmonics WEC (\cite{weaver2020super}), LUPA WEC float (\cite{beringer2025degrees}), and RM3 WEC float (\cite{neary2014methodology}), by discretizing slanted sections into a finite number of annular cylindrical regions, as we will show in Sec.~\ref{sec:slant}. 
...
The fluid is assumed to be inviscid, irrotational, and incompressible. 
Additionally, the body motion is assumed to be small, and the ratio between the wave amplitude and wavelength is assumed to be less than unity [\cite{mei2005theory}]. 
...
The potential function $\Phi_\mathrm{T}$ is both a function of spatial coordinates $\mathbf{x}$ and time $t$. 
Since only heaving vertically axisymmetric bodies are modeled in this work, the cylindrical coordinate system 
$\mathbf{x}=(r, \theta, z)$ is used, as shown in Fig.~\ref{fig:Diagram}. 
Specifically, the body profile of radius versus depth must be continuous and monotonic,
which excludes cases where the bottom of a fluid region touches a body.
The spacial and temporal dependencies can be separated as 
...
$\lambda_0^e$ is the wave number, and $h$ is the water depth~\cite{chatzigeorgiou2018analytical}. 
...
The details of this are discussed in Appendix~\ref{appA}.
...
where $\vec{C}_{i}^{j} = [C_{i1}^j, C_{i2}^j, ..., C_{i(N^j-1)}^j]$. 
The vector $\vec{b} \in \mathbb{C}^{N_\mathrm{T}}$ consists of sub-vectors $b_m$ such that 
...
The vector $\vec{R}_i^j(r) = [R_{i0}^j(r), R_{i1}^j(r),\dots, R_{i(N^j-1)}^j(r) ]$, 
and is evaluated at $r=a_m$ in each matrix $\mathbf{A}_m$. 
...
Consequently, $A_m \in \mathbb{R}^{(N^{i_m} + N^{i_{m+1}}) \times (2N^{i_m} + 2N^{i_{m+1}})}$, 
while $A_1 \in \mathbb{R}^{(N^{i_1} + N^{i_{2}}) \times (N^{i_1} + 2N^{i_2})}$ 
and $A_M \in \mathbb{C}^{(N^{i_M} + N^{e}) \times (2N^{i_M} + N^e)}$.
...
To determine the number of subdivisions needed to accurately model a slant with a given steepness, we compared the hydrodynamic coefficients from Capytaine and MEEM for a variety of steepnesses and number of subdivisions when modeling a cone. As shown in Fig.~\ref{fig:slope-dependence}, steeper slants (smaller angle $\beta$) require more subdivisions for the same level of accuracy, and the error in the approximation is roughly inversely proportional to the number of subdivisions. Convergence of the added mass with respect to the number of subdivisions is slower than that of damping, as shown by the added mass for an angle of $\pi/12$ being unable to achieve less that $10 \%$ error with the number of subdivisions considered. Meanwhile, less than $5 \%$ error in damping was achieved for all angles when 30 subdivisions were used. While it is possible to achieve more accurate solutions using more subdivisions, the computation cost of modeling these steeper geometries will be higher due to the size of the $\mathbf{A}$ matrix growing with the number of subdivisions, potentially limiting MEEM's computational advantage over the more general BEM solvers.
...
Y. Bimali, Cornell Engineering Learning Initiatives through the Fund for Undergraduate Research on Solutions to Climate Change, the Bill Nye ’77 Award in Undergraduate Research \& Robert A. Cowie ’55 ME; R. McCabe, National Science Foundation Graduate Research Fellowship Program, Cornell Provost's Diversity Fellowship, Cornell Mechanical and Aerospace Engineering McMullen Fellowship; C. Treacy, Sandia LDRD; K. Khanal, Sandia Seedlings program, Sandia LDRD; E. Lo, ELI through the Dean Archer Undergraduate Research Program 
\end{bmhead}
