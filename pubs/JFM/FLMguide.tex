...
After confirming these assumptions, the main objective of the convergence study was to find the dimensionless parameter and $\alpha$ and $\beta$ as a function of them. Then the number of terms needed in each region is predicted by taking the maximum of the number required for a converged added mass and that required for a converged radiation damping (Fig.~\ref{fig:nmk-predict-flowchart}).
...
\begin{figure}
    ...
    \caption{Show discontinuities, motivate sampling, \textcolor{red}{Need to add a0, b), .., so we can refer to each plot within this figure.}}
...
\ifdefined\DISSERTATION
    ...
    Authors may include details of the contributions made by each author to the manuscript.
    \end{bmhead}
\else
