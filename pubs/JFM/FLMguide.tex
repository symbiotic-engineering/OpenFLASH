...
\subsection{Inaccuracies in Velocity Potential} While the hydrodynamic coefficients computed using MEEM may approach their true values for a slanted geometry, inaccuracies in the velocity potential are still expected locally. This is shown in Fig~\ref{fig:placeholder}, where the radiated velocity potential for a discretized body is compared with that of a slanted body. Inaccuracies are the largest at the points that are within the slanted body and outside the discretized body. This is due to different boundary conditions being enforced for each problem. Meanwhile, the velocity potential is most accurate at the corners due to this being where the wetted surface of the discretized and slanted bodies meets. In the far field, the velocity potentials of the problems have good agreement.   

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{example-image-a}
    \caption{Radiated velocity potential of discretized (stepped) and slanted geometries.}
    \label{fig:placeholder}
\end{figure}
