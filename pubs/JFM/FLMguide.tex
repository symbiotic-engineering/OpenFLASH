...
Define the local parameters of a target region to be those extracted from its local geometry, such as its fluid height, radial width, or the fluid heights of its neighbors. Cumulative parameters are overall metrics combining the geometries of other regions, such as the total radial distance to the center/exterior region. Each dimensionless parameter chosen to characterize the target's convergence should be either a local or cumulative parameter, as opposed to some quantity specific to a non-neighboring region's local geometry or the number of other regions. This allows the convergence study results to be generalizable to a configuration with any number of regions. Sets of dimensionless parameters are not unique, but good sets will depend more directly on each parameter individually instead of exhibiting complicated coupling between them.
