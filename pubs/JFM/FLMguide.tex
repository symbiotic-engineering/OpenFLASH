...
Note that, for large $M$, the geometry shown in Fig.~\ref{jfm:fig:Diagram} can approximate a more general heaving surface-piercing axisymmetric bodies, as we will show in Sec.~\ref{jfm:sec:slant}.
...
However, to find the hydrodynamic forces on the floating body, only the incident and radiated potential are needed, as explained in Sec.~\ref{jfm:Hydrodynamic Forces}. 
Thus, the main focus of Sec.~\ref{jfm:Matching Across Fluid Boundaries} and~\ref{jfm:Block Matrix Structure} is solving the radiation problem. 
...
These solutions are chosen since they satisfy Eq.~\ref{Laplace equation internal region}-\ref{jfm:eq:R-ODE} for their corresponding region. 
...
As shown in Table~\ref{jfm:tab:MEEM-eigenfunctions}, when specifying the frequency $\omega$ and geometry of the cylindrical rings, 
...
\subsection{Matching Across Fluid Boundaries }\label{jfm:Matching Across Fluid Boundaries}
...
\begin{equation}\label{jfm:potential matching}
...
\begin{equation}\label{jfm:velocity matching}
...
\begin{equation}\label{jfm:no radial velocity}
    \frac{\partial \phi^\mathrm{t}}{\partial r}(a,z)=0 
    \text{ for } -d_\mathrm{s} \le z \le -d_\mathrm{t}.
\end{equation}
\noindent Using Eq.~\ref{jfm:potential matching}-\ref{jfm:no radial velocity}, 
...
\subsection{Block Matrix Structure }\label{jfm:Block Matrix Structure}
...
This is consistent with what is shown in Table~\ref{jfm:tab:MEEM-A-matrix}. 
Tables~\ref{jfm:tab:MEEM-b_m-vector-case-1-simplified}-\ref{jfm:tab:MEEM-b_m-vector-case-3-simplified} 
...
which can be substituted into the expressions of Table~\ref{jfm:tab:MEEM-eigenfunctions} 
...
\subsection{Hydrodynamic and Hydrostatic Forces }\label{jfm:Hydrodynamic Forces}
...
\begin{equation}\label{jfm:eq: added mass matrix}
...
By applying Eq.~\ref{jfm:eq:excitation force} for $q=1,2,\dots, Q$, we can find the heave excitation force coefficient vector $\vec{X} \in \mathbb{C}^Q$, which is a column vector with elements $X_q$ in the $q$th entry.
...
Frequency $\omega$ and wavenumber $\lambda_0^e$ are related by the dispersion relation in Table \ref{jfm:tab:MEEM-eigenfunctions}, which is monotonic and depends on $h$. Extreme frequency values (i.e. extreme $\lambda_0^e$ and/or $h$) push solutions towards edge cases, enabling simplification or requiring modifications to avoid numerical errors.

\subsubsection{Low Frequency}
Empirically, as $\omega \to 0$, the components of $\vec x$ exhibit the following asymptotic behavior: $\text{Re}(C^{i_m}_{1n_m}), \text{Re}(C^{i_m}_{2n_m})$ behave like $K_1 + K_2\ln(\lambda_0^e h)$ (for some nonzero constants $K_1, K_2$, different for each coefficient/region) for $n_m = 0$ and approach nonzero constants for $n_m>0$. $\text{Im}(C^{i_m}_{1n_m}), \text{Im}(C^{i_m}_{2n_m})$ approach nonzero constants for $n_m=0$ and zero for $n_m>0$ (Fig.~\ref{jfm:fig: extreme-frequencies}).
...
\hline
\end{tabular}
\caption{Typical Bessel functions ($I_\nu(z), K_\nu(z)$) exhibit approximately exponential growth or decay. Exponentially scaled Bessel functions counteract this and span only a few orders of magnitude for the reasonable range of inputs. Examples of the new form of the radial eigenfunctions are shown here.} \label{jfm:table:exp-bessels} \end{table}
...
To prevent overflow, we derive an analytical limit in section~\ref{jfm:sec:high-freq-limit} which allows for larger values of $\lambda_0^eh>\cosh^{-1}(\rm f_{max})h/d_2$ before overflow.
...
\subsubsection{Nonlinear Solve of $\lambda_k^e$}\label{jfm:sec:nonlin-solve}
...
In particular, the A-matrix is block tri-diagonal, opening up the possibility for speedups via advanced algorithms (see section~\ref{jfm:sec:compute-time}).
\begin{figure}[h!]
    \centering
...
\begin{figure}
    ...
    \label{jfm:fig:placeholder-2}
\end{figure}

After confirming these assumptions, the main objective of the convergence study was to find the dimensionless parameter and $\alpha$ and $\beta$ as a function of them. Then the number of terms needed in each region is predicted by taking the maximum of the number required for a converged added mass and that required for a converged radiation damping (Fig.~\ref{jfm:fig:nmk-predict-flowchart}).
...
The set of dimensionless parameters that characterize a region's convergence can be divided into local and cumulative parameters. Local parameters are extracted from the geometry of the region itself, such as its fluid height, radial width, or the ratio of its fluid height to those of its neighbors. Cumulative parameters are overall metrics combining the geometries of other regions, such as total radial distance to the center/exterior region. Neither parameter type should depend on a specific other non-neighboring region or the number of other regions, because they should be generalizable to a configuration with any number of regions. Sets of dimensionless parameters are not unique, but good sets will have more direct dependencies on each parameter individually.
[TODO transition/section reorder?]

To find the convergence of a configuration's hydrodynamic coefficients with respect to $N^{i_m}$, compute them with all $N^{i_k}$ at a high number $N_{max}$ for their ``true values.'' Then, fix all $N^{i_k}, k\neq m$ to $N_{max}$, compute hydrodynamic coefficients with $N^{i_m}$ over a range of $[1, 2, \dots, N_{big}], N_{big}<N_{max}$, and find the errors $\epsilon = \frac{\text{val}-\text{true}}{\text{true}}$ of the hydrodynamic coefficients.

To find the dependence of $\alpha$ and $\beta$ on geometry, we considered configurations with three body regions (and one exterior region), and found a good set of dimensionless parameters. After a dominant dimensionless parameter was found, it was kept constant in the search for the next most important parameter. After a set was decided, we kept as many dimensionless parameters constant as possible when measuring a single parameter's effect. Effects were quantified by fitting an $\alpha$ and $\beta$ value to each configuration (geometry + target region) and plotting them against the dimensionless parameter.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{example-image-a}
    \caption{matshow of correlation matrix labeled with self/inner/outer neighbor regions}
    \label{jfm:fig:placeholder-3}
\end{figure}
...
\subsection{Inaccuracies in Velocity Potential} While the hydrodynamic coefficients computed using MEEM may approach their true values for a slanted geometry, inaccuracies in the velocity potential are still expected locally. This is shown in Fig~\ref{fig:potential comparison plots}, where the radiated velocity potential for a discretized body (using MEEM) is compared with that of a slanted body (using Capytaine). Inaccuracies are the largest at the points that are within the discretized body and outside the slanted body, as shown in Fig~\ref{fig:potential comparison plots}c) and d). This is due to different boundary conditions being enforced for each problem. Meanwhile, the velocity potential is most accurate at the corners due to this being where the wetted surface of the discretized and slanted bodies meets. This is shown by the points in Fig~\ref{fig:potential comparison plots}e) and f), where the value of the potential along the slanted outline is compared to that of the stepped outline. In general, the error in the potential along the slanted outline fluctuates far more than that of the stepped outline, with errors up to 8\%. This implies that integration along the stepped outline may lead to more accurate hydrodynamic coefficients, despite the surface being integrated over not being the true slanted geometry. In the far field, the velocity potentials of the problems have good agreement.   
...
\begin{figure}
    ...
    \caption{Show discontinuities, motivate sampling, 
    %\textcolor{red}{Need to add a0, b), .., so we can refer to each plot within this figure.}
    }
...
To determine the number of subdivisions needed to accurately model a slant with a given steepness, we compared the hydrodynamic coefficients from Capytaine and MEEM for a variety of steepnesses and number of subdivisions. As shown in Fig.~\ref{jfm:fig:slope-dependence}, steeper slants (smaller angle $\beta$) require more subdivisions for the same level of accuracy, and the error in the approximation is roughly inversely proportional to the number of subdivisions. Convergence of the added mass with respect to the number of subdivisions appears to be slower than that of damping. It is achievable for damping to approach a similar level of error/difference to that of purely cylindrical (non-slanted) configurations. However, while added mass can approach acceptable ($<3\%$) errors, the errors are still larger than those in the cylindrical case. 
...
\hline
\end{tabular}
\caption{Dependencies of various objects on $\omega$ and which regions are heaving. Changes to $\mathbf{A}$ and $\vec b$ due to $\omega$ only occur in rows corresponding to the boundary between regions $i_M$ and $e$.} \label{jfm:tab:reusable-dependence} \end{table}
...
%%%
Plugging the eigenvalue definitions into Eq.~\ref{jfm:eq:equal-args}, we see that the $n_m$th Bessel term in the $m$th region can be 
...
\else
    ...
    \bibliography{jfm,zotero-meem-refs}
\fi
