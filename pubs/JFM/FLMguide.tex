...
\begin{equation}
    ...
    \end{matrix}.\right.
\end{equation}
Additionally, each sub-matrix $\mathbf{A}_m$ can be broken into a matrix dependent on radial eigenfunctions $\mathbf{A}_{m,r}$ 
and a matrix dependent on vertical eigenfunctions $ \mathbf{A}_{m,z}$, 
where $\mathbf{A}_m = \mathbf{A}_{m,r} \odot \mathbf{A}_{m,z}$, as shown in Appendix~\ref{sec:Forms of Matrix A}.

After constructing the $\mathbf{A}$ matrix and $\vec{b}$ vector that correspond to the problem's geometry and wave conditions, 
numerically solving the $\mathbf{A} \vec{x}=\vec{b}$ equation yields the unknown eigencoefficients $\vec{x}$, 
