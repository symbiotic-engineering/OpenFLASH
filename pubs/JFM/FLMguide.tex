...
\subsection{Linear Hydrodynamics and eigen-functions}
...
\begin{table}
    \centering
    \begin{tabular}{|>{\centering\arraybackslash}p{0.085\linewidth}|>{\centering\arraybackslash}p{0.26\linewidth}|>{\centering\arraybackslash}p{0.34\linewidth}|>{\centering\arraybackslash}p{0.20\linewidth}|} \hline 
 ...
 $\displaystyle \begin{cases} \lambda_{0}^{e} \tanh(\lambda_{0}^{e} h)= \omega^2/g, & n=0 \\ \lambda_{n}^{e} \tan(\lambda_{n}^{e} h) = -\omega^2/g, & n \geq 1\\ \end{cases} $\\\hline
    \end{tabular}
    \caption{Equations for the potential (homogeneous and particular), eigen-functions (radial and vertical), and eigen-values for each region. $i$ and $e$ denote internal and external regions, respectively.}
...
\begin{equation}\label{coupling integral i_M and e}
    [\boldsymbol{\mathcal{Z}}^{i_M e} ]_{nk}=- \sqrt{\frac{1}{2N_k}} \left( \frac{\sin ((d_M-h) (\lambda_k^{e}-\lambda_n^{i_M}))}{(\lambda_k^{e}-\lambda_n^{i_M})} + \frac{\sin ((d_M-h) (\lambda_k^{e}+\lambda_n^{i_M}))}{(\lambda_k^{e}+\lambda_n^{i_M})}\right).
\end{equation}
Note that Eq.~\ref{coupling integral i_m and i_m+1} and~\ref{coupling integral i_M and e} can simplify considerably when $n$ and/or $k$ are zero. 
...
Frequency $\omega$ and wavenumber $\lambda_0^e$ are related by the dispersion relation in Table \ref{tab:MEEM-eigen-functions}, which is monotonic and involves $h$. Extreme frequency values (i.e. extreme $\lambda_0^e$ and/or $h$) push solutions towards edge cases, enabling simplification or requiring modifications to avoid numerical errors.
\subsubsection{Low Frequency}
As $\lambda_0^e \to 0$, the contribution from the first eigenfunction in each region increases while contributions from later eigenfunctions die out. [Log plot of contribution from different coefficients in the innermost region, as an example? Real and imag differ because one converges and one diverges. Similar style to plot for infinite frequency approach contribution/coefficient distribution. Or coefficients are normalized to their value at m0 = 1.]
...
Lastly, damping and excitation phase approach zero as $\lambda_n^e$ approaches infinity. Mathematically, this is evident from the matrix formulation: the Hankel functions $H_0^1$ are the only Bessel functions we work with that give imaginary values for real inputs, so they supply the only imaginary elements to the A matrix. [TODO FINISH UP]
...
The added mass and radiation damping from MEEM and Capytaine, an open-source BEM software, were compared over a range of frequencies for a CorPower-like WEC geometry from \cite{faedo2021energy}. This is shown in Fig.~\ref{fig:MEEM nonslant validation}. Note that the geometry is simplified in the MEEM and Capytaine models so that the geometry consists of two heaving surface-piercing compound annual cylinders that are rigidly fixed to one another. In reality, the Cor-power WEC geometry also consists of a slanted section. This will be accounted for later in Sec.~\ref{sec:slant}. The geometry settings of this comparison were set as follows: $a_1=1.25 \text{m}$, $a_2=4.2 \text{m}$, $d_1=14.45 \text{m}$, $d_2=2.05 \text{m}$, and $h=50 \text{m}$. The series in each region was truncated to 150 for MEEM ($N^{i_1}=N^{i_2}=N^{e}=150$) and 6840 panels were used in Capytaine. The hydrodynamic coefficients from MEEM and Capytaine are on average within 1\% of one another. Small discrepancies can be attributed to Capytaine needing more panels to be completely converged, a consequence of Capytaine's slow convergence with the number of panels. In Sec.~\ref{sec:convergence}, the dependence of MEEM's convergence on the geometry will be discussed more in depth.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{figs/MEEM_vs_Capytaine_Nonslant_Validation.pdf}
    \caption{Added mass and radiation damping coefficients from MEEM and Capytaine for CorPower-like WEC geometry without slanted region.}
...
\int^{-d_\mathrm{t}}_{-h} \frac{\partial \phi^\mathrm{t}_h}{\partial r}Z^\mathrm{t}_{k}(z)dz \,
\fcolorbox{red}{white}{$\displaystyle {- (\tan \beta_\mathrm{t}) \int^{-d_\mathrm{t}}_{-d_\mathrm{s}} 
...
= \int^{-d_\mathrm{s}}_{-h} \frac{\partial \phi^\mathrm{s}_p}{\partial r}Z^\mathrm{t}_{k}(z)dz \, \fcolorbox{blue}{white}{$\displaystyle {+ (\tan \beta_\mathrm{t}) \int^{-d_\mathrm{t}}_{-d_\mathrm{s}} 
...
Without the boxed terms, Eq.~\ref{separated velocity matching slant integral} is identical to the flat case (Eq.~\ref{separated velocity matching integral}). The red box populates corrections to $\mathbf{A}$ and the blue box to $\vec b$. The contents of the blue box should be replaced with $0$ if region $\mathrm{t}$ is not heaving. As before, all integrals in Eq.~\ref{separated velocity matching slant integral} have closed forms.
...
For steeply slanted configurations, the corrected formulation is observed to converge (with respect to the number of subdivisions) to the true slant value faster than the standard formulation. This effect is especially noticeable for damping. % TODO: Insert refs to figures here.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figs/MEEM_vs_Capytaine_Slant_Validation.pdf}
        \caption{Validation results.}
        \label{fig:validation_sub}
    \end{subfigure}
    \hfill % This adds horizontal space between the two images
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figs/Slant_Convergence.pdf}
        \caption{Convergence of each method for $\omega=?$.}
        \label{fig:convergence_sub}
    \end{subfigure}    
    \caption{Comparison of hydrodynamic coefficients for the CorPower-like WEC. Note, this is dummy data.}
    \label{fig:combined_hydro_results}
\end{figure}
...
\end{itemize}
Major time complexities for functions used in MEEM are listed in Table~\ref{tab:time-complexities}, and examples of their distribution/dominance given the same $N^{i_m}$ per region are given in Fig~\ref{fig:time-dominance}. For low region counts, coupling integrals/Bessel functions [TODO fix timing issues to find which one] dominate. For large region counts and terms per region, as is commonly encountered in slant approximations, the matrix solve will eventually dominate. Without a clever computation scheme, this scales cubically with the number of unknown coefficients (matrix side length).

For comparison, while Capytaine effectively solves a matrix system whose side length scales with panel count, it does so quadratically. Its solve time is dominated by that quadratic relation. Given that MEEM's matrix is sparse and its construction can be decomposed into element-wise products, there likely also exists some way to reduce the order of its matrix solve.
...
\begin{figure}[htbp]
    ...
    \caption{Left: Distribution of function computation times for the geometry in Fig.~\ref{fig:MEEM nonslant validation}, varying the terms per region. Right: The function taking the most computation time for combinations of region count and terms per region, assuming the same number of terms per region.}
