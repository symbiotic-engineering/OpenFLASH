...
\begin{equation}\label{jfm:eq:R-ODE}
    \gls{sym-r}^2\frac{\mathrm{d}^2 \gls{sym-R-r}}{\mathrm{d}\gls{sym-r}^2}+\gls{sym-r}\frac{\mathrm{d} \gls{sym-R-r}}{\mathrm{d}\gls{sym-r}} + (\gls{sym-lambda}^2\gls{sym-r}^2-\gls{sym-nu}^2)\gls{sym-R-r}=0
\end{equation}
where $\gls{sym-R-r}(\gls{sym-r})$, $\gls{sym-Theta}(\gls{sym-theta})$, and $\gls{sym-Z}(\gls{sym-z})$ are eigenfunctions, and $\gls{sym-lambda}$ and $\gls{sym-nu}$ are eigenvalues.
...
Each sub-matrix $\gls{sym-mathbf-A-m}$ contains the left-hand-sides of Eq.~\ref{jfm:eq:potential matching vector equations} and~\ref{jfm:eq:velocity matching vector equations} (excluding the eigencoefficients) when applying them to the $\gls{sym-m}$th boundary.
...
\begin{aligned}
    ...
    &= 2 \gls{sym-pi} \gls{sym-rho-w} \sum_{\gls{sym-m} \in \gls{sym-mathcal-M-p}} \int_{\gls{sym-a-m}}^{\gls{sym-a-m-1}} \ _{}^{\gls{sym-q}}\gls{sym-phi-i-m}(\gls{sym-r},-\gls{sym-d-m}) \ r  \ dr \\
    &= 2 \gls{sym-pi} \gls{sym-rho-w} \sum_{\gls{sym-m} \in \gls{sym-mathcal-M-p}} \Bigg[
    \int_{\gls{sym-a-m}}^{a_{{m+1}}} \ _{}^{\gls{sym-q}}\gls{sym-phi-mathrm-p-i-m}(\gls{sym-r},-\gls{sym-d-m})\, \gls{sym-r}\, dr \\
    & + \sum_{\gls{sym-n-m}=0}^{\gls{sym-N-i-m}-1} 
    \ _{}^{\gls{sym-q}}\gls{sym-C-1-n-m-i-m} \ Z_{n_m}^{i_m}(-\gls{sym-d-m})
    \int_{\gls{sym-a-m}}^{a_{{m+1}}} \ R_{1n_m}^{i_m}(\gls{sym-r})\, \gls{sym-r}\, dr \\
    & + \sum_{\gls{sym-n-m}=0}^{\gls{sym-N-i-m}-1} 
    \ _{}^{\gls{sym-q}}\gls{sym-C-2-n-m-i-m} \ Z_{n_m}^{i_m}(-\gls{sym-d-m})
