...
The velocity potentials for the interior regions must satisfy
...
\begin{equation}\label{jfm:eq:No flux BC at wetted surface}
    \left.
\frac{\partial\gls{sym-phi-i-m}}{\partial \gls{sym-z}} \right|_{\gls{sym-z}=-\gls{sym-d-m}}=
    \begin{cases}
        0 & \text{ when region } \gls{sym-m-region} \text{ is fixed} \\
        1 & \text{ when region } \gls{sym-m-region} \text{ is heaving}
    \end{cases}
\end{equation}
and
\begin{equation}\label{jfm:eq:No flux BC at sea floor internal}
    \left.
\frac{\partial\gls{sym-phi-i-m}}{\partial \gls{sym-z}} \right|_{\gls{sym-z}=-\gls{sym-h}}=0,   
\end{equation}
while the velocity potential for the exterior region must satisfy
...
\frac{\partial\gls{sym-phi-e}}{\partial \gls{sym-z}} \right) \right|_{\gls{sym-z}=0}= 0,
\end{equation}
and
\begin{equation}\label{jfm:eq:No flux BC at sea floor external}
    \left.
\frac{\partial\gls{sym-phi-e}}{\partial \gls{sym-z}} \right|_{\gls{sym-z}=-\gls{sym-h}}=0.   
\end{equation}
The velocity potential in the interior regions can be written as the superposition of a homogeneous part $\gls{sym-phi-i-m-mathrm-h}$, which corresponds to the first case of Eq.~\ref{jfm:eq:No flux BC at wetted surface} when the body in region $\gls{sym-m-region}$ is fixed, and a particular part $\gls{sym-phi-mathrm-p-i-m}$, which corresponds to the second case of Eq.~\ref{jfm:eq:No flux BC at wetted surface} when the body in region $\gls{sym-m-region}$ is heaving with unit amplitude velocity.
...
The velocity potentials can be written as a product of eigenfunctions $\gls{sym-phi} (\gls{sym-r}, \gls{sym-theta}, \gls{sym-z})= \gls{sym-R-r}\gls{sym-Theta}(\gls{sym-theta})\gls{sym-Z}(\gls{sym-z})$, where $\gls{sym-phi}$ is $\gls{sym-phi-i-m}$ and $\gls{sym-phi-e}$ for the interior and exterior regions, respectively.
...
\begin{equation}\label{jfm:eq:Z-ODE}
    \frac{\mathrm{d}^2 \gls{sym-Z}}{\mathrm{d}\gls{sym-z}^2}-\gls{sym-lambda}^2\gls{sym-Z}=0
\end{equation}
\begin{equation}\label{jfm:eq:Theta-ODE}
    \frac{\mathrm{d}^2 \gls{sym-Theta}}{\mathrm{d}\gls{sym-theta}^2}+\gls{sym-nu-circumferential}^2\gls{sym-Theta}=0
\end{equation}
\begin{equation}\label{jfm:eq:R-ODE}
    \gls{sym-r}^2\frac{\mathrm{d}^2 \gls{sym-R-r}}{\mathrm{d}\gls{sym-r}^2}+\gls{sym-r}\frac{\mathrm{d} \gls{sym-R-r}}{\mathrm{d}\gls{sym-r}} + (\gls{sym-lambda}^2\gls{sym-r}^2-\gls{sym-nu-circumferential}^2)\gls{sym-R-r}=0
\end{equation}
where $\gls{sym-R-r}$, $\gls{sym-Theta}(\gls{sym-theta})$, and $\gls{sym-Z}(\gls{sym-z})$ are eigenfunctions, and $\gls{sym-lambda}$ and $\gls{sym-nu-circumferential}$ are eigenvalues.
Since we are considering a vertically axisymmetric geometry and only heave motion, $\gls{sym-Theta}=1$ and $\gls{sym-nu-circumferential}=0$.
...
\begin{equation}
    \gls{sym-N-n-e} = \frac{1}{2}\left(1+\frac{\gls{sym-f-n-e}}{2\gls{sym-lambda-n-e-e}\gls{sym-h}}  \right)~
    \textrm{ where }
    \gls{sym-f-n-e} = 
    \begin{cases}
        \sinh(2 \gls{sym-lambda-0-e} \gls{sym-h}), & \gls{sym-n-e}=0 \\ \sin(2\gls{sym-lambda-n-e-e}\gls{sym-h}), & \gls{sym-n-e}\geq1
    \end{cases}.
\end{equation}
%
\section{Matching Equations}\label{jfm:appB}
Since both the value of the potential and fluid velocity must match at the boundary of neighboring regions, there are a total of $2\gls{sym-M-regions}$ matching equations.
However, these equations alone are not enough to solve for all eigencoefficients since the number of unknowns is greater than the number of equations ($\gls{sym-N-T} >2\gls{sym-M-regions}$).
...
To derive a system of linear algebraic equations in terms of the eigencoefficients, we first multiply both sides of Eq.~\ref{jfm:eq:potential matching} and~\ref{jfm:eq:velocity matching} by a vertical eigenfunction, $\gls{sym-Z-mathrm-s-n-mathrm-s}(\gls{sym-z})$ for potential matching and $\gls{sym-Z-mathrm-t-n-mathrm-t}(\gls{sym-z})$ for velocity matching, and integrate over the fluid-fluid boundary to get
\begin{equation}\label{jfm:eq:potential matching integral}
    \int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-s}}\gls{sym-phi-mathrm-t}(\gls{sym-a-generic-radius},\gls{sym-z}) \gls{sym-Z-mathrm-s-n-mathrm-s}(\gls{sym-z}) \mathrm{d}\gls{sym-z}=\int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-s}}\gls{sym-phi-mathrm-s}(\gls{sym-a-generic-radius},\gls{sym-z}) \gls{sym-Z-mathrm-s-n-mathrm-s}(\gls{sym-z}) \mathrm{d}\gls{sym-z}
\end{equation}
for potential matching and
\begin{equation}\label{jfm:eq:velocity matching integral}
    \int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-s}}\frac{\partial \gls{sym-phi-mathrm-t}}{\partial \gls{sym-r}}(\gls{sym-a-generic-radius},\gls{sym-z}) \gls{sym-Z-mathrm-t-n-mathrm-t}(\gls{sym-z}) \mathrm{d}\gls{sym-z}=\int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-s}}\frac{\partial \gls{sym-phi-mathrm-s}}{\partial \gls{sym-r}}(\gls{sym-a-generic-radius},\gls{sym-z}) \gls{sym-Z-mathrm-t-n-mathrm-t}(\gls{sym-z}) \mathrm{d}\gls{sym-z}.
\end{equation}
...
We want orthogonality to occur on the shorter side for potential matching because we have no information about the potential on the interval $\gls{sym-z}\in(-\gls{sym-d-mathrm-s},-\gls{sym-d-mathrm-t})$ which is included in the taller side's orthogonal domain.
On the other hand, we want orthogonality to occur on the taller side for velocity matching because we not only have information about the velocity on the interval $\gls{sym-z}\in(-\gls{sym-d-mathrm-s},-\gls{sym-d-mathrm-t})$ from boundary condition Eq.~\ref{jfm:eq:no radial velocity}, but we have no other way to enforce this condition without incorporating it in the present equation.
Specifically, incorporating the condition can be accomplished because Eq.~\ref{jfm:eq:no radial velocity} states that the integrand of the left-hand-side of Eq.~\ref{jfm:eq:velocity matching integral} is zero in the interval $-\gls{sym-d-mathrm-s} \le \gls{sym-z} \le -\gls{sym-d-mathrm-t}$, allowing us to change the integration bounds on the left-hand-side of Eq.~\ref{jfm:eq:velocity matching integral} from $-\gls{sym-h} \le \gls{sym-z} \le -\gls{sym-d-mathrm-s}$ to $-\gls{sym-h} \le \gls{sym-z} \le -\gls{sym-d-mathrm-t}$:
\begin{equation}\label{jfm:eq:modified velocity matching integral}
    \int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-t}}\frac{\partial \gls{sym-phi-mathrm-t}}{\partial \gls{sym-r}}(\gls{sym-a-generic-radius},\gls{sym-z})\gls{sym-Z-mathrm-t-n-mathrm-t}(\gls{sym-z}) \mathrm{d}\gls{sym-z}=\int_{-\gls{sym-h}}^{-\gls{sym-d-mathrm-s}}\frac{\partial \gls{sym-phi-mathrm-s}}{\partial \gls{sym-r}}(\gls{sym-a-generic-radius},\gls{sym-z}) \gls{sym-Z-mathrm-t-n-mathrm-t}(\gls{sym-z}) \mathrm{d}\gls{sym-z}.
\end{equation}
It is not desired to change the bounds of the right-hand-side because $\gls{sym-phi-mathrm-s}$ is not meaningfully defined on $\gls{sym-z}\in(-\gls{sym-d-mathrm-s},-\gls{sym-d-mathrm-t})$.
...
where 
\begin{equation}
    \gls{sym-vec-R-1-ell}(\gls{sym-r})=[\gls{sym-R-10-ell}(\gls{sym-r}), \gls{sym-R-11-ell}(\gls{sym-r}),\dots, \gls{sym-R-1-N-ell-1-ell}(\gls{sym-r}) ]
\end{equation}
and
\begin{equation}
     \gls{sym-vec-R-2-ell}(\gls{sym-r})=[\gls{sym-R-20-ell}(\gls{sym-r}), \gls{sym-R-21-ell}(\gls{sym-r}),\dots, \gls{sym-R-2-N-ell-1-ell}(\gls{sym-r}) ] 
\end{equation}
 are row vectors of radial eigenfunctions;
\begin{equation}
    \gls{sym-vec-C-1-ell}=[\gls{sym-C-10-ell}, \gls{sym-C-11-ell},\dots, \gls{sym-C-1-N-ell-1-ell}]^{\top}
\end{equation}
and
\begin{equation}
    \gls{sym-vec-C-2-ell}=[\gls{sym-C-20-ell}, \gls{sym-C-21-ell},\dots, \gls{sym-C-2-N-ell-1-ell}]^{\top}
\end{equation}
 are column vectors of eigencoefficients; and
\begin{equation}
\begin{aligned}
    &\gls{sym-vec-Z-ell}(\gls{sym-z})=[\gls{sym-Z-0-ell}(\gls{sym-z}),\gls{sym-Z-1-ell}(\gls{sym-z}),\dots, \gls{sym-Z-N-ell-1-ell}(\gls{sym-z})]^{\top} 
\end{aligned}
\end{equation}
...
Section~\ref{jfm:sec:Block Matrix Structure} shows how to organize Eq.~\ref{jfm:eq:potential matching vector equations} and~\ref{jfm:eq:velocity matching vector equations} for the $\gls{sym-M-regions}$ boundaries so that all eigencoefficients can be solved for simultaneously.
...
Eq.~\ref{jfm:coupling integral i_m and i_m+1} holds for $1 \le \gls{sym-m-region} \le \gls{sym-M-regions}-1$. For matching at the interface between the outermost inner region $\gls{sym-i-M}$ and the exterior region $e$, a different expression is used.
...
\begin{equation}\label{jfm:coupling integral i_M and e}
    [\gls{sym-boldsymbol-mathcal-Z-i-M-e} ]_{\gls{sym-n-M}\gls{sym-n-e}}=
...
Each sub-matrix $\gls{sym-mathbf-A-m}$ contains the left-hand-sides of Eq.~\ref{jfm:eq:potential matching vector equations} and~\ref{jfm:eq:velocity matching vector equations} (excluding the eigencoefficients) when applying them to the $\gls{sym-m-region}$th boundary.
...
The matrix $\gls{sym-mathbf-A-m}$ can be written as 
...
and $\gls{sym-mathbf-I}^{\gls{sym-N-i-m} \times \gls{sym-N-i-m}}$ is a $\gls{sym-N-i-m} \times \gls{sym-N-i-m}$ identity matrix.
%
\begin{landscape}
\begin{table}
    \centering
    ...
    \\ \hline \hline 
      ...
      \shortstack{
        ...
        $\gls{sym-N-i-m-1}$
        & 
        $ \mathbf{1}_{\gls{sym-N-i-m-1}1} \gls{sym-vec-R-1-i-m}$ 
        & 
        $ \mathbf{1}_{\gls{sym-N-i-m-1}1} \gls{sym-vec-R-2-i-m}$ 
        & 
        $\mathbf{1}_{\gls{sym-N-i-m-1}1}  \frac{\partial}{\partial \gls{sym-r}} \gls{sym-vec-R-1-i-m-1}$ 
        & 
        $\mathbf{1}_{\gls{sym-N-i-m-1}1}  \frac{\partial}{\partial \gls{sym-r}} \gls{sym-vec-R-2-i-m-1}$ 
        \\ \hline
    \end{tabular}
    \caption[\gls{acr-meem} $\gls{sym-mathbf-A-m-r}$ sub-matrix when $\gls{sym-d-m}>\gls{sym-d-m-1}$]{\gls{acr-meem} $\gls{sym-mathbf-A-m-r}$ sub-matrix when $\gls{sym-d-m}>\gls{sym-d-m-1}$ ($\gls{sym-i-m} = \gls{sym-mathrm-s}$ and $\gls{sym-i-m-1} = \gls{sym-mathrm-t}$). 
...
\begin{table}
    ...
    $\mathbf{1}_{\gls{sym-N-i-m-1}1} \gls{sym-vec-R-2-i-m}$ 
    & 
    $\mathbf{1}_{\gls{sym-N-i-m-1}1} \gls{sym-vec-R-1-i-m-1}$ 
    & 
    $ \mathbf{1}_{\gls{sym-N-i-m-1}1}  \gls{sym-vec-R-2-i-m-1}$ 
    \\ \hline
      ...
      $\gls{sym-N-i-m}$
      & 
      $\mathbf{1}_{\gls{sym-N-i-m}1} \frac{\partial}{\partial \gls{sym-r}} \gls{sym-vec-R-1-i-m}$ 
      & 
      $\mathbf{1}_{\gls{sym-N-i-m}1} \frac{\partial}{\partial \gls{sym-r}} \gls{sym-vec-R-2-i-m}$ 
      & 
      $ \mathbf{1}_{\gls{sym-N-i-m}1} \gls{sym-vec-R-1-i-m-1}$ 
      & 
      $ \mathbf{1}_{\gls{sym-N-i-m}1} \gls{sym-vec-R-2-i-m-1}$ 
      \\ \hline
    \end{tabular}
    \caption[\gls{acr-meem} $\gls{sym-mathbf-A-m-r}$ sub-matrix when $\gls{sym-d-m}<\gls{sym-d-m-1}$]{\gls{acr-meem} $\gls{sym-mathbf-A-m-r}$ sub-matrix when $\gls{sym-d-m}<\gls{sym-d-m-1}$ ($\gls{sym-i-m} = \gls{sym-mathrm-t}$ and $\gls{sym-i-m-1} = \gls{sym-mathrm-s}$). 
    Note all radial eigenfunctions and their derivatives are evaluated at $\gls{sym-r}=\gls{sym-a-m}$.
    }
...
\begin{table}
    ...
    \\ \hline \hline 
      ...
      \shortstack{
        ...
        $\gls{sym-N-i-m}$ 
        & 
        $(\gls{sym-h}-\gls{sym-d-m}) \gls{sym-mathbf-I}^{\gls{sym-N-i-m} \times \gls{sym-N-i-m}}$ 
        & 
        $(\gls{sym-h}-\gls{sym-d-m}) \gls{sym-mathbf-I}^{\gls{sym-N-i-m} \times \gls{sym-N-i-m}}$ 
        & 
      ...
      \shortstack{
        ...
        $\gls{sym-boldsymbol-mathcal-Z-i-m-1-i-m}$ 
        & 
        $-(\gls{sym-h}-\gls{sym-d-m-1}) \gls{sym-mathbf-I}^{\gls{sym-N-i-m-1} \times \gls{sym-N-i-m-1}}$ 
        & 
        $-(\gls{sym-h}-\gls{sym-d-m-1}) \gls{sym-mathbf-I}^{\gls{sym-N-i-m-1} \times \gls{sym-N-i-m-1}}$ 
        \\ \hline
    \end{tabular}
    \caption[\gls{acr-meem} $\gls{sym-mathbf-A-m-z}$ sub-matrix when $\gls{sym-d-m}>\gls{sym-d-m-1}$]{\gls{acr-meem} $\gls{sym-mathbf-A-m-z}$ sub-matrix when $\gls{sym-d-m}>\gls{sym-d-m-1}$ ($\gls{sym-i-m} = \gls{sym-mathrm-s}$ and $\gls{sym-i-m-1} = \gls{sym-mathrm-t}$). 
    }
...
\begin{table}
    ...
    \\ \hline
      ...
      \shortstack{
        ...
        \\ \hline
    \end{tabular}
    \caption[\gls{acr-meem} $\gls{sym-mathbf-A-m-z}$ sub-matrix when $\gls{sym-d-m}<\gls{sym-d-m-1}$]{\gls{acr-meem} $\gls{sym-mathbf-A-m-z}$ sub-matrix when $\gls{sym-d-m}<\gls{sym-d-m-1}$ ($\gls{sym-i-m} = \gls{sym-mathrm-t}$ and $\gls{sym-i-m-1} = \gls{sym-mathrm-s}$).}
    \label{jfm:tab:MEEM-A_mz-matrix-case-2}
\end{table}
\end{landscape}
%
\section{Added Mass and Damping}\label{jfm:appC}
We will define the velocity of the $\gls{sym-q-dof}$th body as 
...
Thus, if regions consisting of the $\gls{sym-q-dof}$th body are heaving, $\gls{sym-hat-v-q}=1$. 
...
\begin{aligned}
    ...
    &= 2 \gls{sym-pi} \gls{sym-rho-w} \sum_{\gls{sym-m-region} \in \gls{sym-mathcal-M-p}} \int_{\gls{sym-a-m}}^{\gls{sym-a-m-1}} \ _{}^{\gls{sym-q-dof}}\gls{sym-phi-i-m}(\gls{sym-r},-\gls{sym-d-m}) \ r  \ dr \\
    &= 2 \gls{sym-pi} \gls{sym-rho-w} \sum_{\gls{sym-m-region} \in \gls{sym-mathcal-M-p}} \Bigg[
    \int_{\gls{sym-a-m}}^{a_{{m+1}}} \ _{}^{\gls{sym-q-dof}}\gls{sym-phi-mathrm-p-i-m}(\gls{sym-r},-\gls{sym-d-m})\, \gls{sym-r}\, dr \\
    & + \sum_{\gls{sym-n-m}=0}^{\gls{sym-N-i-m}-1} 
    \ _{}^{\gls{sym-q-dof}}\gls{sym-C-1-n-m-i-m} \ Z_{n_m}^{i_m}(-\gls{sym-d-m})
    \int_{\gls{sym-a-m}}^{a_{{m+1}}} \ R_{1n_m}^{i_m}(\gls{sym-r})\, \gls{sym-r}\, dr \\
    & + \sum_{\gls{sym-n-m}=0}^{\gls{sym-N-i-m}-1} 
    \ _{}^{\gls{sym-q-dof}}\gls{sym-C-2-n-m-i-m} \ Z_{n_m}^{i_m}(-\gls{sym-d-m})
    \int_{\gls{sym-a-m}}^{a_{{m+1}}} \ R_{2n_m}^{i_m}(\gls{sym-r})\, \gls{sym-r}\, dr
    \Bigg]
\end{aligned}
\end{equation}
where all symbols written with a left superscript $\gls{sym-q-dof}$ denote 
symbols specific to solving the radiation problem when only the $\gls{sym-q-dof}$th body is moving.
...
\begin{aligned}\relax
    ...
    & = \int_{\gls{sym-a-m}}^{a_{{m+1}}} \gls{sym-R-1-n-m-i-m}(\gls{sym-r})\, \gls{sym-r}\, dr \\
    & = \left\{\begin{matrix}
...
\begin{aligned}\relax
    ...
    & = \int_{\gls{sym-a-m}}^{a_{{m+1}}} \gls{sym-R-2-n-m-i-m}(\gls{sym-r})\, \gls{sym-r}\, dr \\
    & = \left\{\begin{matrix}
    \frac{1}{8} \left(2 a_{{m+1}}^2 \ln \left(\frac{a_{{m+1}}}{\gls{sym-a-m}}\right)-a_{{m+1}}^2+\gls{sym-a-m}^2\right) 
    & 
    \text{ for } \gls{sym-n-m}=0 \\[2ex]
 \displaystyle\frac{
        \gls{sym-a-m} \gls{sym-mathrm-K-1}(\gls{sym-a-m} \gls{sym-lambda-n-m-i-m})-a_{{m+1}} \gls{sym-mathrm-K-1}(a_{{m+1}} \gls{sym-lambda-n-m-i-m})
        }{
        \gls{sym-lambda-n-m-i-m} \gls{sym-mathrm-K-0}(\gls{sym-a-m} \gls{sym-lambda-n-m-i-m})
        } 
 &
  \text{ for } \gls{sym-n-m} \ge 1
\end{matrix}.\right.
