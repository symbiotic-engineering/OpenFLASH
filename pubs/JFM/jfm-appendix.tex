...
The velocity potentials can be written as a product of eigenfunctions $\gls{sym-phi} (\gls{sym-r}, \gls{sym-theta}, \gls{sym-z})= \gls{sym-R-r}\gls{sym-Theta}(\gls{sym-theta})\gls{sym-Z}(\gls{sym-z})$, where $\gls{sym-phi}$ is $\gls{sym-phi-i-m}$ and $\gls{sym-phi-e}$ for the internal and external regions, respectively.
...
\end{equation}
where $\gls{sym-R-r}(\gls{sym-r})$, $\gls{sym-Theta}(\gls{sym-theta})$, and $\gls{sym-Z}(\gls{sym-z})$ are eigenfunctions, and $\gls{sym-lambda}$ and $\gls{sym-nu-circumferential}$ are eigenvalues.
...
Eq.~\ref{jfm:coupling integral i_m and i_m+1} holds for $1 \le \gls{sym-m-region} \le \gls{sym-M-regions}-1$. For matching at the interface between the outermost inner region $\gls{sym-i-M}$ and the external region $e$, a different expression is used.
